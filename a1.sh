...
comm_count4_8ms=0
comm_count8_16ms=0
comm_count16_128ms=0
comm_count128_1s=0
comm_count1s=0

for d in `grep "commit time" $trace_file | egrep -v $ckpt_wri_pids | cut -d' ' -f6`
do
 comm_count=$(($comm_count+1))
 if [ $d -le 1000000 ]; then
  comm_count_1ms=$(($comm_count_1ms+1))
  continue
 fi
 if [ $d -gt 1000000 -a $d -le 2000000 ]; then
  comm_count1_2ms=$(($comm_count1_2ms+1))
  continue
 fi
 if [ $d -gt 2000000 -a $d -le 4000000 ]; then
  comm_count2_4ms=$(($comm_count2_4ms+1))
  continue
 fi
 if [ $d -gt 4000000 -a $d -le 8000000 ]; then
  comm_count4_8ms=$(($comm_count4_8ms+1))
  continue
 fi
 if [ $d -gt 8000000 -a $d -le 16000000 ]; then
  comm_count8_16ms=$(($comm_count8_16ms+1))
  continue
 fi

 if [ $d -gt 16000000 -a $d -le 128000000 ]; then
  comm_count16_128ms=$(($comm_count16_128ms+1))
  continue
 fi
 if [ $d -gt 128000000 -a $d -le 1000000000 ]; then
  comm_count128_1s=$(($comm_count128_1s+1))
  continue
 fi
 if [ $d -gt 1000000000 ]; then
   comm_count1s=$(($comm_count1s+1))
  continue
 fi

done

echo "Commit count total: "$comm_count
echo "<1ms count: "$comm_count_1ms
echo "1-2ms count: "$comm_count1_2ms
echo "2-4ms count: "$comm_count2_4ms
echo "4-8ms count: "$comm_count4_8ms
echo "8-16ms count: "$comm_count8_16ms
echo "16-128ms count: "$comm_count16_128ms
echo "128ms-1s count: "$comm_count128_1s
echo ">1s count:  "$comm_count1s
echo
